...
<head>
	<meta charset="UTF-8">
	<title>Listado de Académicos</title>
	<style>
...
<body>
	...
	<h2>Listado de Usuarios con rol de Académico</h2>
	<table>
		<thead>
			<tr>
				<th>ID</th>
		...
		<tbody>
			@foreach($academics as $academic)
				<tr>
					<td>{{ $academic->id }}</td>
					<td>{{ $academic->full_name ?? '-' }}</td>
					<td>{{ $academic->department->name ?? '-' }}</td>
					<td>{{ $academic->department && $academic->department->institution ? $academic->department->institution->name : '-' }}</td>
					<td>{{ strip_tags($academic->biography ?? '-') }}</td>
					<td>
						@if ($academic->is_active)
							<span style="color: #283C2A; font-weight: bold;">Activo</span>
						@else
							<span style="color: #888; font-weight: bold;">Inactivo</span>
						@endif
					</td>
				</tr>
			@endforeach
