<template>
    ...
    <LayoutMain>
        ...
        <CardBox isForm>
            <DataForm :equipmentTypes="equipmentTypes" :academics="academics" :departments="departments" />
            <template #footer>
                <BaseButtons>
                    <BaseButton :routeName="`${routeName}index`" :icon="mdiClose" color="lightDark" label="Cancelar" />
                    <BaseButton @click="updateForm" :icon="mdiCheck" :processing="processing" color="info"
                        label="Guardar" />
...
const { form, processing, updateForm, destroyForm } = useEquipment(props.routeName, props.facilityEquipment);
</script>
