...
const form = useForm({
    id: props.educationalLevel.data?.id,
    name: props.educationalLevel.data?.name,
});

const { setLoading } = useLoading();

const updateForm = () => {
    form.put(route(`${props.routeName}update`, props.educationalLevel.data?.id), {
        onBefore: () => {
            setLoading(true);
        },
        onError: () => {
...
const deleteForm = () => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.educationalLevel.data?.id));
        }
    });
};
</script>