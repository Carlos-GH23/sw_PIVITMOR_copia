...
const form = useForm({
    id: props.educationalModality.data?.id,
    name: props.educationalModality.data?.name,
});

const { setLoading } = useLoading();

const updateForm = () => {
    form.put(route(`${props.routeName}update`, props.educationalModality.data?.id), {
        onBefore: () => {
            setLoading(true);
        },
        onError: () => {
...
const deleteForm = (id) => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.educationalModality.data?.id));
        }
    });
};
</script>