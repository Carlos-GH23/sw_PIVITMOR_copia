<template>
    ...
    <LayoutMain>
        ...
        <SearchBar @apply-filters="applyFilters" @clear-filters="clearFilters" v-model:search="filters.search"
            v-model:rows="filters.rows" :routeName="routeName" title="área" :total="socialSectors.meta.total" />
        <CardBox v-if="socialSectors.data.length > 0">
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr v-for="item in socialSectors.data" :key="item.id">
        ...
        <pagination :links="socialSectors.meta.links" :total="socialSectors.meta.total" :to="socialSectors.meta.to"
            :from="socialSectors.meta.from" />
    </LayoutMain>
</template>
<script setup>
...
const deleteForm = (socialSector) => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            router.delete(route(`${props.routeName}destroy`, socialSector));
        }
    });
};
</script>