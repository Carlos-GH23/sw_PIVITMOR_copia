...
const form = useForm({
    id: props.socialSector.data?.id,
    name: props.socialSector.data?.name,
    description: props.socialSector.data?.description,
});

const { setLoading } = useLoading();

const updateForm = () => {
    form.put(route(`${props.routeName}update`, props.socialSector.data?.id), {
        onBefore: () => {
            setLoading(true);
        },
        onError: () => {
...
const deleteForm = () => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.socialSector.data?.id));
        }
    });
};
</script>