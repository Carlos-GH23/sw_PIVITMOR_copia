<template>
    ...
    <loading v-model:active="isLoading" :can-cancel="false" :is-full-page="true" loader="dots" color="#1447E6" />
    <LayoutMain>
        ...
        <SearchBar @apply-filters="applyFilters" @clear-filters="clearFilters" v-model:search="filters.search"
            ...
            :total="equipmentTypes.meta.total" />

        <CardBox v-if="equipmentTypes.data.length > 0">
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr v-for="item in equipmentTypes.data" :key="item.id">
        ...
        <pagination :links="equipmentTypes.meta.links" :total="equipmentTypes.meta.total" :to="equipmentTypes.meta.to"
            :from="equipmentTypes.meta.from" />
    </LayoutMain>
</template>
<script setup>
...
const deleteForm = (equipmentType) => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            router.delete(route(`${props.routeName}destroy`, equipmentType));
        }
    });
};
</script>