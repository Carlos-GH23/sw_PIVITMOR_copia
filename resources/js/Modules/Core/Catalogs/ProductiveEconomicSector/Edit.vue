...
const form = useForm({
    id: props.productiveEconomicSector.data?.id,
    name: props.productiveEconomicSector.data?.name,
    description: props.productiveEconomicSector.data?.description,
});

const { setLoading } = useLoading();

const updateForm = () => {
    form.put(route(`${props.routeName}update`, props.productiveEconomicSector.data?.id), {
        onBefore: () => {
            setLoading(true);
        },
        onError: () => {
...
const deleteForm = () => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.productiveEconomicSector.data?.id));
        }
    });
};
</script>