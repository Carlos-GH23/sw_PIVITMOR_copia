...
const form = useForm({
    id: props.copaesAccreditation.data?.id,
    name: props.copaesAccreditation.data?.name,
    description: props.copaesAccreditation.data?.description,
});

const { setLoading } = useLoading();

const updateForm = () => {
    form.put(route(`${props.routeName}update`, props.copaesAccreditation.data?.id), {
        onBefore: () => {
            setLoading(true);
        },
        onError: () => {
...
const deleteForm = (id) => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.copaesAccreditation.data?.id));
        }
    });
};
</script>