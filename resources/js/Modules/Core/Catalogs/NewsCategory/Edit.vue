...
const form = useForm({
    id: props.newsCategory.data?.id,
    name: props.newsCategory.data?.name,
    description: props.newsCategory.data?.description,
});

const { setLoading } = useLoading();

const updateForm = () => {
    form.put(route(`${props.routeName}update`, props.newsCategory.data?.id), {
        onBefore: () => {
            setLoading(true);
        },
        onError: () => {
...
const deleteForm = (id) => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.newsCategory.data?.id));
        }
    });
};
</script>