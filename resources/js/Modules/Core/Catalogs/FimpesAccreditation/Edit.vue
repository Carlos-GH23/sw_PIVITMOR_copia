...
const form = useForm({
    id: props.fimpesAccreditation.data?.id,
    name: props.fimpesAccreditation.data?.name,
});

const { setLoading } = useLoading();

const updateForm = () => {
    form.put(route(`${props.routeName}update`, props.fimpesAccreditation.data?.id), {
        onBefore: () => {
            setLoading(true);
        },
        onError: () => {
...
const deleteForm = (id) => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.fimpesAccreditation.data?.id));
        }
    });
};
</script>