<template>
    ...
    <LayoutMain>
        ...
        <SearchBar @apply-filters="applyFilters" @clear-filters="clearFilters" v-model:search="filters.search"
            v-model:rows="filters.rows" :routeName="routeName" title="Tipo de Servicio"
            :total="serviceTypes.meta.total" />

        <CardBox v-if="serviceTypes.data.length > 0">
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr v-for="item in serviceTypes.data" :key="item.id">
                        ...
                        <td data-label="acciones">
                            <BaseButtons>
                                <BaseButton color="info" :icon="mdiPencil" small :routeName="`${routeName}edit`"
                                    :parameter="item.id" title="Editar tipo de servicio" />
        ...
        <pagination :links="serviceTypes.meta.links" :total="serviceTypes.meta.total" :to="serviceTypes.meta.to"
            :from="serviceTypes.meta.from" />
    </LayoutMain>
</template>
