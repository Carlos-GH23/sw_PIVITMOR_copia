<template>
    ...
    <LayoutAuthenticated>
        ...
        <SearchBar @apply-filters="applyFilters" @clear-filters="clearFilters" v-model:search="filters.search"
            v-model:rows="filters.rows" :routeName="routeName" title="categoría de servicio"
            :total="serviceCategories.meta.total" />

        <CardBox v-if="serviceCategories.data.length > 0">
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr v-for="item in serviceCategories.data" :key="item.id">
                        ...
                        <td data-label="Acciones">
                            <BaseButtons>
                                <BaseButton color="info" :icon="mdiPencil" small :routeName="`${routeName}edit`"
                                    :parameter="item.id" title="Editar categoría de servicio" />
        ...
        <pagination :links="serviceCategories.meta.links" :total="serviceCategories.meta.total"
            :to="serviceCategories.meta.to" :from="serviceCategories.meta.from" />
    </LayoutAuthenticated>
</template>
<script setup>
