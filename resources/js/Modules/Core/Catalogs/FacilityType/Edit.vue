...
const form = useForm({
    ...
    description: props.facilityType.description,
});

const saveForm = () => {
    form.put(route(`${props.routeName}update`, props.facilityType.id));
};

const deleteForm = () => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.facilityType.id));
        }
    });
};
</script>