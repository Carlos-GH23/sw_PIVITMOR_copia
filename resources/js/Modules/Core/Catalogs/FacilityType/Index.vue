<template>
    ...
    <LayoutMain>
        ...
        <SearchBar @apply-filters="applyFilters" @clear-filters="clearFilters" v-model:search="filters.search"
            ...
            :total="facilityTypes.meta.total" />

        <CardBox v-if="facilityTypes.data.length > 0">
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr v-for="item in facilityTypes.data" :key="item.id">
        ...
        <pagination :links="facilityTypes.meta.links" :total="facilityTypes.meta.total" :to="facilityTypes.meta.to"
            :from="facilityTypes.meta.from" />
    </LayoutMain>
</template>
<script setup>
...
const deleteForm = (facilityType) => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            router.delete(route(`${props.routeName}destroy`, facilityType));
        }
    });
};
</script>