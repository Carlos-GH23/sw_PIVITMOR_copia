<template>
    ...
    <LayoutMain>
        ...
        <SearchBar @apply-filters="applyFilters" @clear-filters="clearFilters" v-model:search="filters.search"
            v-model:rows="filters.rows" :routeName="routeName" title="Actividad" :total="activities.meta.total" />

        <CardBox v-if="activities.data.length > 0">
            <table>
                <thead>
                    <tr>
        ...
        <pagination :links="activities.meta.links" :total="activities.meta.total" :to="activities.meta.to"
            :from="activities.meta.from" />
    </LayoutMain>
</template>
<script setup>
...
const deleteForm = (activity) => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            router.delete(route(`${props.routeName}destroy`, activity));
        }
    });
};
</script>