...
const form = useForm({
    id: props.researchArea.data?.id,
    name: props.researchArea.data?.name,
    description: props.researchArea.data?.description,
    order: props.researchArea.data?.order,
});

const { setLoading } = useLoading();

const updateForm = () => {
    form.put(route(`${props.routeName}update`, props.researchArea.data?.id), {
        onBefore: () => {
            setLoading(true);
        },
        onError: () => {
...
const deleteForm = () => {
    messageConfirm().then((res) => {
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.researchArea.data?.id));
        }
    });
};
</script>